    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Dynamic Assets</h1>
            <p className="text-sm text-slate-400">Build personalized images for email campaigns</p>
          </div>
        </div>
        <button
          onClick={() => startNew()}
          ...
          <div className="grid grid-cols-2 gap-4">
            {presets.map((preset, i) => (
              <button
                ...
                className="rounded-xl bg-slate-800/50 border border-slate-700/50 p-4 text-left hover:border-indigo-500/30 hover:bg-indigo-500/5 transition-all group"
              >
        ...
        <>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h3 className="text-lg font-medium text-white mb-1">No templates yet</h3>
              <p className="text-sm text-slate-400 mb-4">Create your first personalized image template or start from a preset.</p>
            </div>
          ) : (
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
              {templates.map((t) => (
                <div
                  key={t.id}
                  className="rounded-xl bg-slate-800/50 border border-slate-700/50 overflow-hidden hover:border-slate-600/50 transition-colors group cursor-pointer"
                ...
                >
                  ...
                  <div className="flex items-center justify-between border-t border-slate-700/30 px-4 py-3">
                    <div>
                      <h4 className="text-sm font-medium text-white">{t.name}</h4>
                      <p className="text-xs text-slate-500">{t.width}x{t.height}</p>
                    </div>
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-indigo-600 to-indigo-500 px-3 py-1.5 text-xs font-medium text-white hover:from-indigo-500 hover:to-indigo-400 transition-all"
                  >
                ...
                {Object.entries(previewParams).map(([key, value]) => (
                  <div key={key}>
                    ...
                    <input
                      ...
                      className="w-full rounded-lg bg-slate-900/50 border border-slate-700/50 px-3 py-1.5 text-sm text-white focus:outline-none focus:border-indigo-500/50"
                    />
                  </div>
                ))}
              </div>
            ...
            <div className="rounded-xl bg-slate-800/50 border border-slate-700/50 p-4 space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg bg-slate-900/50 border border-slate-700/50 px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500/50"
                />
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="h-9 w-12 rounded-lg border border-slate-700/50 cursor-pointer"
                  />
                  <input
                    type="text"
                    value={editBg}
                    onChange={(e) => setEditBg(e.target.value)}
                    className="flex-1 rounded-lg bg-slate-900/50 border border-slate-700/50 px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500/50"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl bg-slate-800/50 border border-slate-700/50 p-4">
              ...
              <div className="space-y-2 max-h-[500px] overflow-y-auto pr-1">
                ...
                {editLayers.map((layer, i) => (
                  <div key={i} className="rounded-lg bg-slate-900/50 border border-slate-700/30 p-3 space-y-2">
                    <div className="flex items-center justify-between">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      <div>
                        ...
                        <input type="number" value={layer.x} onChange={(e) => updateLayer(i, { x: Number(e.target.value) })} className="w-full rounded bg-slate-800 border border-slate-700/50 px-2 py-1 text-xs text-white" />
                      </div>
                      <div>
                        <label className="text-[10px] text-slate-500">Y</label>
                        <input type="number" value={layer.y} onChange={(e) => updateLayer(i, { y: Number(e.target.value) })} className="w-full rounded bg-slate-800 border border-slate-700/50 px-2 py-1 text-xs text-white" />
                      </div>
                      <div>
                        <label className="text-[10px] text-slate-500">W</label>
                        <input type="number" value={layer.width || ''} onChange={(e) => updateLayer(i, { width: Number(e.target.value) })} className="w-full rounded bg-slate-800 border border-slate-700/50 px-2 py-1 text-xs text-white" />
                      </div>
                      <div>
                        <label className="text-[10px] text-slate-500">H</label>
                        <input type="number" value={layer.height || ''} onChange={(e) => updateLayer(i, { height: Number(e.target.value) })} className="w-full rounded bg-slate-800 border border-slate-700/50 px-2 py-1 text-xs text-white" />
                      </div>
                    </div>
                    ...
                    {layer.type === 'text' && (
                      <>
                        <div>
                          ...
                          <input type="text" value={layer.content || ''} onChange={(e) => updateLayer(i, { content: e.target.value })} className="w-full rounded bg-slate-800 border border-slate-700/50 px-2 py-1 text-xs text-white" />
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <div>
                            ...
                            <input type="number" value={layer.fontSize || 24} onChange={(e) => updateLayer(i, { fontSize: Number(e.target.value) })} className="w-full rounded bg-slate-800 border border-slate-700/50 px-2 py-1 text-xs text-white" />
                          </div>
                          <div>
                            <label className="text-[10px] text-slate-500">Weight</label>
                            <select value={layer.fontWeight || 'normal'} onChange={(e) => updateLayer(i, { fontWeight: e.target.value })} className="w-full rounded bg-slate-800 border border-slate-700/50 px-2 py-1 text-xs text-white">
                          ...
                          <div>
                            ...
                            <select value={layer.align || 'left'} onChange={(e) => updateLayer(i, { align: e.target.value as 'left' | 'center' | 'right' })} className="w-full rounded bg-slate-800 border border-slate-700/50 px-2 py-1 text-xs text-white">
                        ...
                        <div>
                          ...
                          <div className="flex gap-2">
                            <input type="color" value={layer.color || '#000000'} onChange={(e) => updateLayer(i, { color: e.target.value })} className="h-7 w-8 rounded border border-slate-700/50 cursor-pointer" />
                            <input type="text" value={layer.color || '#000000'} onChange={(e) => updateLayer(i, { color: e.target.value })} className="flex-1 rounded bg-slate-800 border border-slate-700/50 px-2 py-1 text-xs text-white" />
                          </div>
                        </div>
                      </>
                    )}
                      ...
                      <>
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            ...
                            <select value={layer.shape || 'rectangle'} onChange={(e) => updateLayer(i, { shape: e.target.value as 'rectangle' | 'circle' })} className="w-full rounded bg-slate-800 border border-slate-700/50 px-2 py-1 text-xs text-white">
                          ...
                          <div>
                            ...
                            <input type="number" value={layer.borderRadius || 0} onChange={(e) => updateLayer(i, { borderRadius: Number(e.target.value) })} className="w-full rounded bg-slate-800 border border-slate-700/50 px-2 py-1 text-xs text-white" />
                          </div>
                        </div>
                        <div>
                          <label className="text-[10px] text-slate-500">Fill</label>
                          <div className="flex gap-2">
                            <input type="color" value={layer.fill || '#e5e7eb'} onChange={(e) => updateLayer(i, { fill: e.target.value })} className="h-7 w-8 rounded border border-slate-700/50 cursor-pointer" />
                            <input type="text" value={layer.fill || '#e5e7eb'} onChange={(e) => updateLayer(i, { fill: e.target.value })} className="flex-1 rounded bg-slate-800 border border-slate-700/50 px-2 py-1 text-xs text-white" />
                          </div>
                        </div>
                      </>
                    )}
                    ...
                    {layer.type === 'image' && (
                      <div>
                        ...
                        <input type="text" value={layer.src || ''} onChange={(e) => updateLayer(i, { src: e.target.value })} placeholder="https://..." className="w-full rounded bg-slate-800 border border-slate-700/50 px-2 py-1 text-xs text-white" />
                      </div>
                    )}
                  </div>
                ))}
