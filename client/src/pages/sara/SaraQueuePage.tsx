    ...
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">SkySend Autonomous Reply Agent</p>
            </div>
          </div>
        </div>
        <button
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="rounded-xl bg-gradient-to-br from-violet-600/20 to-violet-600/5 border border-violet-500/20 p-4">
          ...
          <p className="text-2xl font-bold text-white">{stats?.pending_review ?? 0}</p>
        </div>
        <div className="rounded-xl bg-gradient-to-br from-emerald-600/20 to-emerald-600/5 border border-emerald-500/20 p-4">
          ...
          <p className="text-2xl font-bold text-white">{stats?.approved_today ?? 0}</p>
        </div>
        <div className="rounded-xl bg-gradient-to-br from-blue-600/20 to-blue-600/5 border border-blue-500/20 p-4">
          ...
          <p className="text-2xl font-bold text-white">{stats?.sent_today ?? 0}</p>
        </div>
        <div className="rounded-xl bg-gradient-to-br from-slate-600/20 to-slate-600/5 border border-slate-500/20 p-4">
          ...
          <p className="text-2xl font-bold text-white">{stats?.dismissed_today ?? 0}</p>
        </div>
      </div>
      ...
      {stats?.top_intents && stats.top_intents.length > 0 && (
        <div className="rounded-xl bg-slate-800/50 border border-slate-700/50 p-4">
          <h3 className="text-sm font-medium text-slate-400 mb-3">Intent Distribution</h3>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-lg font-medium text-white mb-1">No messages in queue</h3>
        ...
        <div className="space-y-3">
          {messages.map((msg: any) => {
            ...
            return (
              <div
                key={msg.id}
                className="rounded-xl bg-slate-800/50 border border-slate-700/50 overflow-hidden hover:border-slate-600/50 transition-colors"
              >
                {/* Message Header */}
                <div
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {msg.contacts?.company && (
                        <span className="text-xs text-slate-500">@ {msg.contacts.company}</span>
                      )}
                    </div>
                  ...
                  {/* Campaign */}
                  {msg.campaigns?.name && (
                    <span className="text-xs text-slate-500 bg-slate-700/50 rounded px-2 py-1">
                      {msg.campaigns.name}
                    </span>
                  )}
                  ...
                  <div className="border-t border-slate-700/50 p-4 space-y-4">
                    ...
                    {statusFilter === 'pending_review' && (
                      <div className="flex items-center gap-2 pt-2">
                        ...
                        <button
                          ...
                          className="flex items-center gap-2 rounded-lg bg-slate-700 px-4 py-2 text-sm font-medium text-slate-300 hover:bg-red-600/20 hover:text-red-300 hover:border-red-500/30 transition-colors ml-auto"
                        >
                          <XCircle className="h-4 w-4" />
                          Dismiss
                        </button>
                    ...
                    {msg.sara_action && (
                      <div className="flex items-center gap-2 text-xs text-slate-500 pt-1">
                        ...
                        Recommended action: <span className="text-slate-400 font-medium">{msg.sara_action}</span>
                      </div>
                    )}
                  </div>
                )}
