...
export function SmtpAccountsPage() {
  ...
  const [connectingProvider, setConnectingProvider] = useState<'google' | 'azure' | null>(null);

  const handleQuickConnect = async (provider: 'google' | 'azure') => {
    ...
    try {
      ...
      if (error) {
        ...
        toast.success(`Connecting to ${provider === 'google' ? 'Google' : 'Microsoft'}...`);
      }
    } finally {
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="bg-gradient-to-br from-slate-50 to-gray-50 rounded-2xl border border-gray-200 p-6">
        <div className="flex items-start justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Connect your Google or Microsoft email with one click
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {/* Google Connect */}
          <button
            onClick={() => handleQuickConnect('google')}
            disabled={connectingProvider !== null}
            className="group relative overflow-hidden flex items-center gap-4 p-5 bg-white rounded-xl border border-gray-200 hover:border-gray-300 hover:shadow-md transition-all duration-200"
          >
            ...
            <div className="relative flex-1 text-left">
              ...
              <p className="text-sm text-gray-500">Use Google Workspace or Gmail</p>
            </div>
          ...
          {/* Microsoft Connect */}
          <button
            onClick={() => handleQuickConnect('azure')}
            disabled={connectingProvider !== null}
            className="group relative overflow-hidden flex items-center gap-4 p-5 bg-white rounded-xl border border-gray-200 hover:border-gray-300 hover:shadow-md transition-all duration-200"
          >
            <div className="absolute inset-0 bg-gradient-to-r from-blue-50/50 via-green-50/30 to-yellow-50/30 opacity-0 group-hover:opacity-100 transition-opacity" />
            <div className="relative flex items-center justify-center w-12 h-12 bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            <div className="relative flex-1 text-left">
              <p className="font-semibold text-gray-900">Connect Outlook</p>
              <p className="text-sm text-gray-500">Use Microsoft 365 or Outlook</p>
            </div>
            {connectingProvider === 'azure' ? (
              <Spinner size="sm" />
            ) : (
              <ArrowRight className="h-5 w-5 text-gray-400 group-hover:text-gray-600 group-hover:translate-x-1 transition-all" />
            )}
